...
#Count the total misdemeanor complaints by season from 2006 - 2016
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: misdemeanor_count_by_season.py <file1> ", file=sys.stderr)
        exit(-1)
    sc = SparkContext()
    ...
    joined = mapping.reduceByKey(lambda x,y: x+y).sortByKey(ascending=True).saveAsTextFile("misdemeanor_count_by_season.out")
