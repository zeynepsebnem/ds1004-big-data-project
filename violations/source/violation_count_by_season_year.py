...
#Count the total violation complaints by season and year from 2006 - 2016
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: violation_count_by_season_year.py <file1> ", file=sys.stderr)
        exit(-1)
    sc = SparkContext()
    ...
    joined = mapping.reduceByKey(lambda x,y: x+y).sortByKey(ascending=True).saveAsTextFile("violation_count_by_season_year.out")
